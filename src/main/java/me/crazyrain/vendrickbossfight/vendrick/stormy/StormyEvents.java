...
public class StormyEvents implements Listener {
    ...
    public void onVendrickAttack(EntityDamageByEntityEvent e){
        if (e.getEntity().getScoreboardTags().contains("venStorm")){
            ...
            if (chance >= 95){
                plugin.getFightManager().getVendrick().startAttack(0);
                ...
                new BukkitRunnable(){
                    ...
                    public void run() {
                        storm.movement();
                        storm.shootBalls(plugin.getFightManager().getVendrick().getEntity().getLocation().getDirection().normalize(), plugin.getFightManager().getVendrick().getEntity().getLocation());
                    }
                    ...
                    for (Entity e : pulse.getNearbyEntities(0.4, 0.4, 0.4)) {
                        ...
                        try {
                            ...
                            if (en.hasMetadata("Vendrick")){;
                                ...
                                plugin.getFightManager().getHurricane().setDamage(plugin.getFightManager().getHurricane().getDamage() - 1);
                                plugin.getFightManager().getHurricane().setRadius(plugin.getFightManager().getHurricane().getRadius() - 0.5f);
    ...
    public void onPlayerDeath(PlayerDeathEvent e) { //Called if players DO NOT keep their inventory when they die in the fight
        if (plugin.getFightManager().isVenSpawned()){ //If the player dies during the fight
